...
$recipient = $cgi->param('RecipientText');
...
if (!(check_email($recipient))) {
    ...
    print "This E-Mail Address is not valid:<BR>\n";
    print "<B>".last_check()->{reason}, "<\B>\n";
...
if ($begdate > $enddate) {
    ...
    print "The End Date **MUST** be late than Begin Date<BR></HTML>";
    print "<A HREF=\"http://ms01.ebtnet.net/cgi-bin/icall_main.pl\">Return to Previous Page</A></HTML>";
    exit 0;
}
...
$sqlstmt=sprintf("SELECT sn,deliver_time,bounce_time,recipient,reason FROM
                  icall WHERE recipient='%s' AND deliver_time BETWEEN %s AND %s 
                  ORDER BY sn ASC",
                  $recipient, $begdate, $enddate);
